...
# 2. Reconstruct options parser for Part 1
...
# 3. Reconstruct options parser for Part 2
...
for test_idx in range(1, 11):
    ...
    output_md.append("| 题号 | 答案 | 题号 | 答案 | 题号 | 答案 | 题号 | 答案 | 题号 | 答案 |")
    output_md.append("| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |")
    for row in range(10):
        row_str = "|"
        for col in range(5):
    ...
    for q in range(1, 7):
        ...
        for let in ["A", "B", "C", "D"]:
            ...
            output_md.append(f"- ({let}) {eng} ({chi})")
            
        if alt_exprs:
            output_md.append("\n**其他可能表达：**")
            for expr in alt_exprs:
                output_md.append(f"- {expr}")
        output_md.append("\n")
    ...
    for q in range(7, 32):
        ...
        if script_qa:
            ...
            for line in script_qa:
                output_md.append(f"- {line}")
            output_md.append("")
            
        output_md.append("**听力原文与选项：**")
        output_md.append(f"- **问题：** {eng_q} ({chi_q})")
        output_md.append("- **选项：**")
        for let in ["A", "B", "C"]:
            eng = eng_opts.get(let, "")
            chi = chi_opts.get(let, "")
    ...
    for p_idx in range(23):
        ...
        # If Q71 is starting, add Part 4 Header
        if q_start == 71:
        ...
        # Check for alternative expressions in Q3 body
        ...
        # Split Dialogue Transcript vs Vocabulary
        ...
        # Embed graphic image if applicable
        is_graphic = False
        graphic_q_num = None
        ...
        if is_graphic:
            # Look up page number for this question
        ...
        if eng_trans:
            ...
            for line in eng_trans:
                output_md.append(line)
            output_md.append("```\n")
            
        if chi_trans:
            output_md.append("**中文对照：**")
            for line in chi_trans:
                output_md.append(f"> {line}")
            output_md.append("")
        ...
        # Parse and output each sub-question
...
print("Successfully wrote reorganized index to TOEIC_精读解析包/01_听力1000题逐题解析索引.md")

# Write to brain artifact directory as well
...
print("Successfully wrote reorganized index to brain directory.")
